...
class NAutoComplete: NTextField {
    ...
    open var highlightAttributes: [NSAttributedStringKey: AnyObject] = [NSAttributedStringKey(rawValue: NSAttributedStringKey.font.rawValue):UIFont.boldSystemFont(ofSize: 18)]
    ...
    override open func willMove(toSuperview newSuperview: UIView?) {
        ...
        self.addTarget(self, action: #selector(NAutoComplete.textFieldDidChange), for: .editingChanged)
        self.addTarget(self, action: #selector(NAutoComplete.textFieldDidBeginEditing), for: .editingDidBegin)
        self.addTarget(self, action: #selector(NAutoComplete.textFieldDidEndEditing), for: .editingDidEnd)
        self.addTarget(self, action: #selector(NAutoComplete.textFieldDidEndEditingOnExit), for: .editingDidEndOnExit)
        
        NotificationCenter.default.addObserver(self, selector: #selector(NAutoComplete.keyboardWillShow(_:)), name: NSNotification.Name.UIKeyboardWillShow, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(NAutoComplete.keyboardWillHide(_:)), name: NSNotification.Name.UIKeyboardWillHide, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(NAutoComplete.keyboardDidChangeFrame(_:)), name: NSNotification.Name.UIKeyboardDidChangeFrame, object: nil)
    }
    ...
    fileprivate func redrawSearchTableView() {
        ...
        if let tableView = tableView {
            ...
            if self.direction == .down {
                ...
                //-50 because view in front of textfiled
                var tableViewFrame = CGRect(x: -50, y: 0, width: frame.size.width - 4 + 50, height: tableHeight)
                ...
                var shadowFrame = CGRect(x: -50, y: 0, width: frame.size.width - 6 + 50, height: 1)
    ...
    @objc open func textFieldDidChange() {
        ...
        timer?.invalidate()
        timer = Timer.scheduledTimer(timeInterval: 0.8, target: self, selector: #selector(NAutoComplete.typingDidStop), userInfo: self, repeats: false)
...
extension NAutoComplete: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        var cell = tableView.dequeueReusableCell(withIdentifier: NAutoComplete.cellIdentifier)
...
public struct SearchTextFieldTheme {
    ...
    public static func lightTheme() -> SearchTextFieldTheme {
        return SearchTextFieldTheme(cellHeight: 30, bgColor: UIColor (red: 1, green: 1, blue: 1, alpha: 1.0), borderColor: UIColor (red: 0.9, green: 0.9, blue: 0.9, alpha: 1.0), separatorColor: UIColor.clear, font: UIFont.systemFont(ofSize: 18), fontColor: UIColor.black)
    }
    
    public static func darkTheme() -> SearchTextFieldTheme {
        return SearchTextFieldTheme(cellHeight: 30, bgColor: UIColor (red: 0.8, green: 0.8, blue: 0.8, alpha: 1.0), borderColor: UIColor (red: 0.7, green: 0.7, blue: 0.7, alpha: 1.0), separatorColor: UIColor.clear, font: UIFont.systemFont(ofSize: 18), fontColor: UIColor.white)
    }
}
