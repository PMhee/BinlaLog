...
open class TagListView: UIView {
    ...
    @objc open dynamic var textFont: UIFont = UIFont.systemFont(ofSize: 17) {
        didSet {
            for tagView in tagViews {
    ...
    fileprivate func rearrangeViews() {
        ...
        for (index, tagView) in tagViews.enumerated() {
            tagView.maxWidth = self.frame.width
            ...
            if currentRowTagCount == 0 || currentRowWidth + tagView.frame.width > frame.width {
                ...
                if currentRow > lastRow{
                    lastRow = currentRow
                    self.frame = CGRect(x: self.frame.origin.x, y: self.frame.origin.y, width: self.frame.width, height: (CGFloat(23*CGFloat(currentRow))))
